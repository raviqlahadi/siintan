...
class Compensation extends Admin_Controller
{

    private $name = null;
    private $parent_page = 'setting';
    private $current_page = 'admin/compensation';
    private $form_data = null;
    private $data_type = null;
    private $label = null;
    private $block_header = "Kompensasi";
    ...
    public function __construct()
    {
        ...
        $this->label = $this->label();
        $this->load->model(array('m_compensations'));
    }
    ...
    public function index()
    {
        ...
        if (!isset($land_id) || $land_id == null) {
            redirect('admin/land');
        } else {
            $this->load->model('m_land');
            $land_data = $this->m_land->getWhere(array('id' => $land_id));
            if (sizeof($land_data) < 1) {

                redirect('admin/land');
            } else {
                $this->data['land_data'] = $land_data[0];
            }
        }
        $limit = $this->input->get('limit');
        $page = ($this->uri->segment(4)) ? ($this->uri->segment(4) - 1) : 0;
        $tabel_cell =  ['id', 'type','area','price'];
        ...
        $fetch['where'] = array('land_id' => $land_id);
        ...
        $pagination['total_records'] =  $this->m_compensations->fetch($fetch, true);
        //set pagination
        //var_dump($pagination);
        ...
        $this->data['id'] = $land_id;
        $this->data['full_total_price'] = $full_total;
        ...
        $this->data["header"] = "tabel compensation";
        $this->data["sub_header"] = 'klik tombol aksi untuk melakukan melakukan aksi lain';

        $this->render("admin/compensation/content");
    }
    ...
    public function create($id)
    {
        ...
        $this->data['parent_page'] = $this->current_page . '?land_id=' . $id;
        $this->data["block_header"] = $this->block_header;
        $this->data["header"] = "tambah compensation";
        $this->data["sub_header"] = 'tekan tombol simpan ketika telah memasukan data';
        $this->render("admin/compensation/create");
    }

    public function insert($data)
    {
        $insert = $this->m_compensations->add($data);
        return $insert;
    }
    ...
    public function edit($id = null)
    {
        ...
        $w['id'] = $id;
        $form_value = $this->m_compensations->getWhere($w);
        if ($form_value == false) {
            redirect($this->current_page . '?land_id=' . $land_id);
        } else {
        ...
        $this->data['breadcrumb'] = $this->setBreadcrumb();
        $this->data['form_data'] = $form;
        $this->data['form_action'] = site_url($this->current_page . '/edit/' . $id . '?land_id=' . $land_id);
        $this->data['name'] = $this->name;
        $this->data['parent_page'] = $this->current_page . '?land_id=' . $land_id;
        $this->data["block_header"] = $this->block_header;
        $this->data["header"] = "ubah compensation";
        $this->data["sub_header"] = 'silahkan ubah data yang ingin anda ganti';
        $this->render("admin/compensation/edit");
    }

    public function update($id, $data)
    {
        $insert = $this->m_compensations->update($id, $data);
        return $insert;
    }
    ...
    public function detail($id = null)
    {
        ...
        $this->data["block_header"] = $this->block_header;
        $this->data["header"] = "detail compensation";
        $this->data["sub_header"] = 'halaman Ini Hanya Berisi Informasi Detail Dari Data';
        $this->render("admin/compensation/detail");
    }
    ...
    public function delete($id)
    {
        ...
        $w['id'] = $id;
        $delete = $this->m_compensations->delete($w);
        if ($delete != false) {
    ...
    private function label()
    {
        $name = $this->name;
        $label = [
            "id kompensasi", "tipe ", "luas(m<sup>2</sup>)", "harga"
        ];
        $label_arr = array();
        foreach ($name as $key => $value) {
            $label_arr[$value] = $label[$key];
        }
        $label_arr['land_owner'] = 'nama lahan';
        return $label_arr;
    }
