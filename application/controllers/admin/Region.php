...
class Region extends Admin_Controller
{
    ...
    private $current_page = 'admin/region';
    private $form_data = null;
    private $data_type = null;
    private $label = null;
    private $block_header = "region management";
    ...
    public function index()
    {
        ...
        $tabel_cell =  ["id", "name", "address", "description", "active"];
        ...
        //fetch data from database
        $fetch['select'] = ["id", "name", "address", "description", "active"];
        ...
        $for_table = $this->m_region->fetch($fetch);

        $pagination['total_records'] =  $this->m_region->fetch($fetch, true);
        //set pagination
        //var_dump($pagination);
        ...
        $this->render("admin/region/content");
    }
    ...
    public function create()
    {
        ...
        $this->data["block_header"] = $this->block_header;
        $this->data["header"] = "Tambah region";
    ...
    public function edit($id = null)
    {
        ...
        $w['id'] = $id;
        $form_value = $this->m_region->getWhere($w);
        if ($form_value == false) {
            redirect($this->current_page);
        } else {
    ...
    {
        $insert = $this->m_region->update($id, $data);
        return $insert;
    }
    ...
    public function detail($id = null)
    {
        ...
        $this->data["block_header"] = $this->block_header;
        $this->data["header"] = "Detail region";
    ...
    {
        return [
            "id", "name", "address", "description", "active"
        ];
    }
    ...
    private function label()
    {
        $name = $this->name;
        $label = [
            "id", "kawasan", "alamat", "keterangan", "status"
        ];
        $label_arr = array();
    ...
    {
        $name = $this->name;
        $type =   ["number", "text", "textarea", "textarea", "select"];
        $type_arr = array();
        foreach ($name as $key => $value) {
    ...
    private function form_data($form_value = null)
    {
        ...
        $select['active'] = array('non-active', 'active');
        $name = $this->name;
        unset($name[0]);
        foreach ($name as $key => $value) {
            if ($form_value != null) {
